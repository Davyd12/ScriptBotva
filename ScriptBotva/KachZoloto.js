javascript:(function() {
    ...
    // Порядок по возрастанию: от минимального к максимальному
    const values = ['99999', '999999', '2499999', '24999999'];
    ...
    function createControlPanel() {
        ...
        const oldStyle = document.getElementById('gold-trainer-styles');
        ...
        setElementText(title, '💰 Автопрокачка (v2 Max)');
        ...
        closeBtn.onclick = () => { stopTraining(); panel.remove(); style.remove(); };
    ...
    function findMyMoney() {
        try {
            ...
            const digits = money.split('').filter(c => c >= '0' && c <= '9').join('');
    ...
    function getStatValue(stat) {
        try {
            ...
            const digits = statText.split('').filter(c => c >= '0' && c <= '9').join('');
            return BigInt(digits || "0");
        } catch (e) {
            return 0n;
        }
    ...
    function getAllStatPrices() {
        return allStats.map(stat => {
            ...
            try {
                ...
                const cleanPrice = priceText.split('').filter(c => c >= '0' && c <= '9').join('');
    ...
    function performUpgrade(statInfo, upgradeCount) {
        console.log(`⚡ Стат: ${statInfo.stat}, Расчетное кол-во: ${upgradeCount}`);
        ...
        // НАДЕЖНЫЙ ПОИСК МАКСИМАЛЬНОГО ДОСТУПНОГО ПОРОГА
        let selectedValue = upgradeCount.toString();
        for (let value of values) {
            if (upgradeCount >= BigInt(value)) {
                selectedValue = value; // Идем снизу вверх (от 99k до 24m), оставляя самое большее подходящее
            } else {
                break; // Если расчетное число меньше текущего порога, останавливаемся
            }
        }
        
        console.log(`🎯 Выбрано для ввода в инпут: ${selectedValue}`);
    ...
    createControlPanel();
    console.log('✅ Автопрокачка v2 загружена!');
})();
