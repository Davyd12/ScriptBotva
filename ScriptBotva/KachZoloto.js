javascript:(function() {
    ...
    const allStats = Object.keys(statNames);
    ...
    function createControlPanel() {
        ...
        setElementText(statsTitle, '📊 Выбор характеристик:');
...
const statName = statNames[stat] || 'Неизвестная характеристика';
            setElementText(label, statName);
            label.style.cssText = `
                cursor: pointer;
                font-size: 13px;
                user-select: none;
    ...
    function updateUpgradeInfo(stat, price, count, upgradeCount) {
        ...
        if (upgradeInfo) {
            const statName = statNames[stat] || 'Неизвестная характеристика';
            setElementText(upgradeInfo, `📊 Улучшаем ${statName}: +${upgradeCount} уровней (цена: ${price}, текущее: ${count})`);
